...
if (data != "" && id_anh == "") {
    ...
    for (i in data) {
        ...
        lay.push(label); //se property searched
        marker.bindPopup('Tên đơn vị: ' + ten_dv + "<br/>" + "Mã khách hàng: " + title + "<br/>" + "Công suất: " + cong_suat + "<br/>" + "Số thứ tự: " + stt +"</br><form action='/taianh' method='POST'><input type='hidden' name='id_anh' value='"+id_anh+"'><input type='hidden' name='check_ma_kh' value='" + title + "'><button type='submit' class='btn btn-primary'>Tải ảnh</button></form>");
        markersLayer.addLayer(marker);
    };
} else {
